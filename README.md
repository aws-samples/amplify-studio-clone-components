...
The example intents to provide a method for cloning the UI components and additionally provide a automated method using Amplify CLI hooks to clone an environment . 

The Example uses AWS SDK v3 Amplify UIBuilder library. 
https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/clients/client-amplifyuibuilder/
...
1. create a `pre-add-env.js` file under the `amplify/hooks` folder(if the hooks folder doesn't exist, create the folder).
2. add the following
